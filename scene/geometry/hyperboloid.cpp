...
Hyperboloid::Hyperboloid(QString name, Transform transform, Material *material) {
    this->name = name;
...
bool Hyperboloid::intersect(const Ray &ray, Intersection *intersection) const {
    float radius = 0.5;
    ...
    float a = (rayDirMod[0]*rayDirMod[0]) + (rayDirMod[2]*rayDirMod[2]) - (rayDirMod[1]*rayDirMod[1]);
    float b = 2*((rayDirMod[0]*rayOriMod[0]) + (rayDirMod[2]*rayOriMod[2]) - (rayDirMod[1]*rayOriMod[1]));
    float c = (rayOriMod[0]*rayOriMod[0]) + (rayOriMod[2]*rayOriMod[2]) - (rayOriMod[1]*rayOriMod[1]) - (radius*radius);
...
void Hyperboloid::mapNormal(Intersection &isect) const {
...
glm::vec3 Hyperboloid::sphereInterpolation(Intersection &isect, QImage *attrib) const {
...
glm::vec3 Hyperboloid::getColor(Intersection &isect) const {
...
float Hyperboloid::getAlpha(float y, float py, float qy) const {
    float result = ((y-py)/(qy-py));
    return result;
}

QRgb Hyperboloid::slerp(float alpha, QRgb az, QRgb bz) const {
...
BoundingBox Hyperboloid::calculateAABB() const {
    BoundingBox ret;
    return ret;
}