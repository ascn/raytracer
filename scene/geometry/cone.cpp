...
Cone::Cone(QString name, Transform transform, Material *material) {
    this->name = name;
...
bool Cone::intersect(const Ray &ray, Intersection *intersection) const {
    ...
    float c = (rayOriMod[0]*rayOriMod[0]) + (rayOriMod[2]*rayOriMod[2]) - (rayOriMod[1]*rayOriMod[1]);
...
void Cone::mapNormal(Intersection &isect) const {
...
glm::vec3 Cone::sphereInterpolation(Intersection &isect, QImage *attrib) const {
...
glm::vec3 Cone::getColor(Intersection &isect) const {
...
float Cone::getAlpha(float y, float py, float qy) const {
    float result = ((y-py)/(qy-py));
    return result;
}

QRgb Cone::slerp(float alpha, QRgb az, QRgb bz) const {
