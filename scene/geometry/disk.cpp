...
Disk::Disk(QString name, Transform transform, Material *material) {
...
float Disk::getT(glm::vec3 n, glm::vec3 s, glm::vec3 origin, glm::vec3 direction) const {
...
void Disk::mapNormal(Intersection &isect) const {
...
glm::vec3 Disk::getColor(Intersection &isect) const {
...
glm::vec3 Disk::spInterpolation(Intersection &isect, QImage *attrib) const {
...
float Disk::getAlpha(float y, float py, float qy) const {
    float result = ((y-py)/(qy-py));
    return result;
}

QRgb Disk::slerp(float alpha, QRgb az, QRgb bz) const {
